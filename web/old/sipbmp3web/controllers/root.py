...
volume_form = twf.TableForm('volume_form', action='https://sipb.mit.edu:444/sipbmp3/volume', children=[
...
class RootController(BaseController):
    ...
    @expose()
    def volume(self, **kw):
        if 'SSL_CLIENT_S_DN_Email' in request.environ:
            server = config['sipbmp3.server']
            remctl(server, command=["volume", "set", kw["volume"], 'by='+request.environ['SSL_CLIENT_S_DN_Email']])
        redirect('index')
